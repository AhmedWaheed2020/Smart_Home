...
void I2C_Init_Master(void)
{
	...
	TWSR &= ~((1<<TWPS0) | (1<<TWPS1));					//baud rate
	TWCR = (1<<TWEN);									//enable I2C	                 
}

void I2C_Init_Slave(void)
{
	TWAR = (0x01 << 1) ;								// my address = 0x01
	TWCR = (1<<TWEN);									//enable I2C    	
}
...
void I2C_Send_Slave_Address_Write_Operation(uint8 Slave_Adress)
{
	TWDR = (Slave_Adress << 1);							//Address and write instruction
...
void I2C_Send_Slave_Address_Read_Operation(uint8 Slave_Adress)
{
	TWDR = (Slave_Adress << 1) | (0X01);				// read instruction
	TWCR = (1<<TWINT) | (1<<TWEN);						// Clear TWI interrupt flag,Enable TWI
